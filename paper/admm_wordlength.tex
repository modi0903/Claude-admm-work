...
amplifies it. We fit
\begin{equation}
\log G = a\log\!\frac{1}{1 - \lVert M\rVert_2} + b\,R_{\text{op}} + c,
\qquad
R_{\text{L1}} = d,\;\; R_{\text{Box}} = d^2,\;\; R_{\text{L2}} = \varnothing
\end{equation}
where $G$ is measured loop gain. Leave-one-out cross-validated error: 0.340
(L1), 0.234 (Box), 0.031 (L2) bits, all inside a 0.5-bit criterion.

\paragraph{The confound was tested.} $d$ correlates with conditioning at
$r^2 = 0.94$, so the obvious objection is that it proxies conditioning where
$\log(1/(1-\lVert M\rVert_2))$ saturates. It does not: $\log(\text{cond})$ as
the second regressor gives LOO 1.237 for L1, \emph{worse than baseline}, and
contributes nothing alongside $d$.

\scoped{This relation is empirical. No mechanism is claimed --- see
Section~8.2. The coefficient signs differ between L1 ($+20.6$) and Box
($-20.4$) because $d$ moves in opposite directions with conditioning in the two
operators ($r = +0.969$ and $-0.975$), not because two mechanisms are at work.
Box prefers $d^2$ over $d$ (paired LOO, $t = 2.56$, $p < 0.05$), so the
operators do not share a functional form; several candidates were tried before
this one, so the exponent is fitted, not derived.}
...
$A_{\text{lane}}(W) = bW + c$, with $b \approx 19.6$ LUT/bit.
\end{theorem}
...
\scoped{Random instances already break the 0.5-bit criterion. The cause is not
an exotic corner: \texttt{loop\_gain.py} fits 20-trial \emph{ensemble averages}
at each conditioning point, so the reported figures (0.834 / 0.399 / 0.031
bits) are ensemble-level quantities. A per-instance refit gives 0.979 / 0.611 /
0.339 bits before any attack.}

\textbf{Consequence.} The loop-gain relation is a design-space exploration tool
predicting mean loop gain for an ensemble at fixed conditioning. It is
\emph{not} a per-instance safety guarantee. Every bit figure must be labelled
``ensemble mean over $N$ trials at fixed conditioning''.
...
losing on their ground with their tools. \todo{Quote a per-instance margin for
wordlength selection, or state that per-instance safety requires the prior
art's bounds.}

\subsection{Other limits}
\begin{itemize}
\item Absolute LUT counts depend on the timing constraint --- \texttt{uniform18}
      is 4673 at 20\,ns and 6308 at 15.079\,ns. The ratio survives ($-44.9\%$
      against $-46.6\%$) but tables at different constraints must never be mixed.
...
\item The 0.5-bit criterion is looser than Li et al.'s $<\!1$\,dB
      ($\approx 0.17$ bits). \todo{Justify the criterion rather than asserting it.}
\end{itemize}
...
\begin{itemize}
\item Board bring-up and measured wall power (Section~11 of the plan).
\item Relabel all loop-gain figures as ensemble means.
\item Justify the 0.5-bit criterion.
\item Answer the per-instance margin question.
\end{itemize}
